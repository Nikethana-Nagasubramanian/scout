...
export async function GET(request: Request) {
  ...
  const rows = db.prepare(`
    SELECT applications.id AS application_id, jobs.id AS job_id,
      jobs.company, jobs.title, jobs.location, jobs.apply_url, jobs.canonical_url,
      applications.resume_version_id,
      cover_letters.id AS cover_letter_id, cover_letters.status AS cover_letter_status
    FROM applications
  ...
  return NextResponse.json({
    ...
    applications: rows.map((row) => ({
      ...
      resumePdfUrl: row.resume_version_id ? `${origin}/api/resumes/${row.resume_version_id}/pdf` : null,
      resumeDocxUrl: row.resume_version_id ? `${origin}/api/resumes/${row.resume_version_id}/docx` : null,
